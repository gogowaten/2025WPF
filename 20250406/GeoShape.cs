...
namespace _20250406
{
    ...
    public class GeoShape : Shape
    {
        ...
        private void MyBindShapeRenderTransformBounds()
        {
            var mb = new MultiBinding() { Converter = new MyConvRenderTransformBounds2() };
            ...
            //mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyRenderTransformProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(RenderTransformProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyPenProperty) });
            SetBinding(MyShapeTransformedBoundsProperty, mb);
        }
        ...
        private void MyBindRenderTransform()
        {
            ...
            //mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyGeometryProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyGeometryRenderBoundsProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyCenterXProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyCenterYProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyAngleProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyScaleXProperty) });
            mb.Bindings.Add(new Binding() { Source = this, Path = new PropertyPath(MyScaleYProperty) });
            //SetBinding(MyRenderTransformProperty, mb);
            SetBinding(RenderTransformProperty, mb);
    ...
    public class MyConvRenderTransformBounds2 : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            if (values[0] == null) { return new Rect(); }
            ...
            var clone = geo.Clone();
            var gtf = (TransformGroup)TF;
            RotateTransform rotate = null;
            ScaleTransform scale = null;
            foreach (var item in gtf.Children)
            {
                if (item is RotateTransform r)
                {
                    rotate = r;
                }
                if (item is ScaleTransform s)
                {
                    scale = s;
                }
            }
            clone.Transform = rotate;
            var rr = clone.GetRenderBounds(myPen);
            var rrr = scale.TransformBounds(rr);
    ...
    public class MyConvRenderTransformBounds : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            return TF.TransformBounds(geo.GetRenderBounds(myPen));
        }
